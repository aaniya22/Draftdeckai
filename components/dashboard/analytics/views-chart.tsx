...
export function ViewsChart({ data }: { data: any[] }) {
  ...
  return (
    <Card
      role="region"
      aria-label="Views trend chart showing document views and unique visitors over time"
      className="p-6 glass-effect border border-border/40 h-[400px]"
    >
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-lg font-bold">Views Trend</h3>
          <p className="text-sm text-muted-foreground">
            Document traffic over time
          </p>
        </div>
      </div>
      ...
      <div
        ...
        role="img"
        aria-label="Area chart displaying document views and unique visitors over time"
      >
